...
import com.billy.cc.core.component.CC.getApplication
import com.lzg.extend.BaseResponse
import com.lzg.extend.JsonConvert
import com.lzg.extend.toDisposables
import com.lzy.okgo.OkGo
import com.lzy.okrx2.adapter.ObservableBody
import com.wyl.common_lib.base.BaseViewModel
import com.wyl.common_ui.LOGIN_LOGIN
import com.wyl.common_ui.LOGIN_REGISTER
...
class LoginViewModel : BaseViewModel() {
    ...
    fun register() {
        ...
        OkGo.post<BaseResponse<LoginData>>(LOGIN_REGISTER)
            ...
            .subscribe({
                error.value = getApplication().getString(R.string.login_register_success)
            }, {
                onError(it)
    ...
    private fun invalid(): Boolean = when {
        username.get().isNullOrEmpty() -> {
            error.value = getApplication().getString(R.string.login_empty_invalid_user_name)
            false
        }
        username.get()!!.length < 5 -> {
            error.value = getApplication().getString(R.string.login_error_invalid_user_name)
            false
        }
        pwd.get().isNullOrEmpty() -> {
            error.value = getApplication().getString(R.string.login_empty_invalid_password)
            false
        }
        pwd.get()!!.length < 7 -> {
            error.value = getApplication().getString(R.string.login_error_invalid_password)
            false
        }
        else -> true
    }
